[
    {
        ...
        "details": "项目前后端分离，Go 原生实现高性能接口和 WebSocket，所有数据以 JSON 文件存储，免数据库部署。前端采用 Vue+Vite+TailwindCSS，支持实时群聊、私聊、表情包、文件/音视频消息和 AI 神评论等丰富互动。权限系统基于 JWT，多端自适应响应式设计，移动端体验优良。支持热加载、接口自动化测试、内容管理后台，易扩展、易二次开发。"
    },
    {
        "id": 2,
        "title": "智能内容管理系统（CMS）",
        ...
        "details": "项目采用PHP自研高性能接口服务，前端Vue+ElementPlus。支持文章标签、分类、评论、点赞、附件上传等模块。实现了基于Redis的缓存与消息队列，大幅提升高并发下的响应速度。支持自定义站点配置和多语言。项目部署自动化（Git+WebHook+容器），显著提升运维效率。"
    },
    {
        "id": 3,
        ...
        "url": "",
        "details": "后端Go实现流量埋点服务，PHP负责业务管理与API，前端Vue自适应大屏。支持多租户、权限细分、资源关联。高并发数据采集采用Go原生协程与Redis异步队列。流量统计与安全风控支持自定义规则和告警推送。产品已在实际项目落地，实现数十万数据点稳定采集。"
    },
    {
        "id": 4,
